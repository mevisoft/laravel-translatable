...
trait Translatable
{
    ...
    {
        $this->translations()
            ->when($locales !== null, function (Builder $query) use ($locales) {
                return $query->whereIn($this->getLocaleName(), Arr::wrap($locales));
            })
            ->cursor()
            ->each(function (Model $translation) {
    ...
    {
        return array_map(function ($resolver): TranslationResolver {
            return app($resolver);
        },
            $resolvers
        );
            ...
            ->leftJoin(
                ...
                function (JoinClause $join) use ($locale) {
                    return $join
                        ...
                        ->when($locale, function ($join) use ($locale) {
                            return $join->where($this->getQualifiedLocaleName(), $locale);
                        });
                }
            )
    ...
    public function scopeTranslatedIn(Builder $query, string $locale = null)
    {
        return $query->whereHas(
            ...
            function (Builder $q) use ($locale) {
                $q->where($this->getQualifiedLocaleName(), '=', $locale ?? $this->locale());
            }
        );
    }
    ...
    public function scopeWhereTranslation(Builder $query, string $translationField, $value, ?string $locale = null, string $method = 'whereHas', string $operator = '=')
    {
        return $query->$method(
            'translations',
            function (Builder $q) use ($translationField, $operator, $value, $locale) {
                $q->where($this->qualifyTranslationColumn($translationField), $operator, $value)
                    ->when($locale, function ($q) use ($operator, $locale) {
                        $q->where($this->getQualifiedLocaleName(), $operator, $locale);
                    });
            }
        );
    }
